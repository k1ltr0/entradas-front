...
const Events: React.FC = () => {
  ...
  const loadEvents = async () => {
    ...
    try {
      const response = await fetch('https://entradas-back-220847228031.us-central1.run.app/api/events', {
        method: 'GET',
        headers: {
  ...
  const handleAddVenue = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`https://entradas-back-220847228031.us-central1.run.app/api/events/${selectedEventId}/venues`, {
        method: 'POST',
        headers: {
          'Authorization': `${tokenType} ${token}`,
          'Content-Type': 'application/json'
